...
import { Activity, BarChart3, ChevronLeft, FileJson, FileText, LayoutDashboard, Menu, MoreHorizontal, Plus, RefreshCcw, Search, Settings, Target, Users, X } from 'lucide-react'
import { useCrm } from '../context/CrmContext.jsx'
import { getCompany, getContact, serviceNames } from '../lib/domain.js'

const nav=[
  ['central','Central Comercial',LayoutDashboard],['pipeline','Pipeline',Target],['leads','Leads',Users],['atividades','Atividades',Activity],['reativacao','Reativação',RefreshCcw],['propostas','Propostas',FileText],['importar','Importar do Analista',FileJson],['relatorios','Relatórios',BarChart3],['configuracoes','Configurações',Settings],
]
const titles={
  central:['Central Comercial','Prioridades, oportunidades e metas do período'],pipeline:['Pipeline de Vendas','Oportunidades organizadas por etapa'],leads:['Leads','Contatos e oportunidades comerciais'],atividades:['Atividades e Follow-ups','O que precisa ser feito e quando'],reativacao:['Reativação','Oportunidades antigas com potencial de retorno'],propostas:['Propostas','Versões, validade, implantação e mensalidade'],importar:['Importar do Analista','Revise o JSON antes de criar empresa, oportunidade e proposta'],relatorios:['Relatórios Comerciais','Funil, coorte, receita, MRR e origem'],configuracoes:['Configurações','Metas, serviços, mensagens e dados'],
}
...
export default function Shell({view,setView,onNew,openOpportunity,children}){
  ...
  return <div className={`app-shell ${collapsed?'collapsed':''}`}>
    <aside className={`sidebar ${mobile?'show':''}`}><div className="brand-row"><div className="brand-mark">LG</div><div className="brand-copy"><strong>Luid Gabriel</strong><span>CRM Comercial V2.3.2</span></div><button className="icon-button sidebar-toggle" onClick={toggle} aria-label="Recolher ou expandir menu"><ChevronLeft/></button></div><nav className="sidebar-nav" aria-label="Navegação principal">{nav.map(([id,label,Icon])=><button key={id} className={view===id?'active':''} onClick={()=>choose(id)}><Icon/><span>{label}</span></button>)}</nav><div className="sidebar-note"><b>CRM = comercial</b><br/>Depois do fechamento, a operação segue para o Meu Escritório Digital.</div></aside>
