...
export default function Shell({view,setView,onNew,openOpportunity,children}){
  ...
  const results=q?state.opportunities.filter(o=>{
    const c=getContact(state,o),co=getCompany(state,o)
    return `${c?.name||''} ${c?.phone||''} ${c?.email||''} ${co?.name||''} ${co?.doc||''} ${serviceNames(o)} ${o.stage}`.toLowerCase().includes(q)
  }).slice(0,8):[]
  ...
  return <div className={`app-shell ${collapsed?'collapsed':''}`}>
    <aside className={`sidebar ${mobile?'show':''}`}>
      <div className="brand-row"><div className="brand-mark">LG</div><div className="brand-copy"><strong>Luid Gabriel</strong><span>CRM Comercial V2.3</span></div><button className="icon-button sidebar-toggle" onClick={toggle} aria-label="Recolher ou expandir menu"><ChevronLeft/></button></div>
    ...
    <main className="main-area">
      <header className="topbar">
        <div className="topbar-title"><button className="icon-button mobile-menu" onClick={()=>setMobile(true)} aria-label="Abrir menu"><Menu/></button><div><h1>{title}</h1><p>{sub}</p></div></div>
        <div className="topbar-tools">
          <div className="global-search"><Search/><input aria-label="Busca global" value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar cliente, CNPJ, WhatsApp..."/>{search&&<button className="search-clear" onClick={()=>setSearch('')} aria-label="Limpar busca"><X/></button>}{q&&<div className="search-results" role="listbox">{results.map(o=>{const c=getContact(state,o),co=getCompany(state,o);return <button key={o.id} onClick={()=>openResult(o.id)}><span><b>{c?.name}</b><small>{co?.name||'Sem empresa'} · {serviceNames(o)}</small></span><span className="pill neutral">{o.stage}</span></button>})}{!results.length&&<div className="empty-search">Nenhum resultado.</div>}</div>}</div>
          <select className="period-select" value={uiPeriod} onChange={e=>setUiPeriod(e.target.value)} aria-label="Período global"><option value="today">Hoje</option><option value="7">7 dias</option><option value="30">30 dias</option><option value="month">Este mês</option><option value="all">Todo período</option></select>
          <div className="top-actions"><button className="btn" onClick={()=>choose('atividades')}>Follow-ups</button><button className="btn primary" onClick={onNew}><Plus/>Novo lead</button></div>
        </div>
      </header>
