...
export default function NewOpportunityModal({open,onClose,onCreated,initialContact=null}){
  ...
  React.useEffect(()=>{
    ...
    setAdvanced(false)
    if(initialContact){
      setForm(f=>({...empty,name:initialContact.name||'',phone:initialContact.phone||'',email:initialContact.email||'',city:initialContact.city||'Belém/PA',source:initialContact.source||'Instagram'}))
    }else setForm(empty)
  ...
  return <Modal open={open} onClose={onClose} title={initialContact?'Nova oportunidade':'Novo lead'} subtitle={advanced?'Cadastro comercial completo':'Cadastre o essencial agora e complete o restante depois.'} footer={footer} size={advanced?'lg':'md'}>
    {duplicate&&<div className="notice warning"><b>Contato já cadastrado: {duplicate.name}</b><br/>O CRM criará uma nova oportunidade para esse contato, sem duplicar a pessoa.</div>}
    ...
    <div className="quick-lead-grid">
      ...
      {!advanced&&<label className="field"><span>Serviço</span><select value={primary} onChange={e=>setPrimary(e.target.value)}>{state.settings.services.map(s=><option key={s.id} value={s.id}>{s.name}</option>)}</select></label>}
    </div>
    ...
    {advanced&&<>
      <div className="form-grid two">
        ...
        <label className="field"><span>Cidade/UF</span><input value={form.city} onChange={e=>patch('city',e.target.value)}/></label>
        <label className="field"><span>Tipo</span><select value={form.type} onChange={e=>patch('type',e.target.value)}><option>MEI</option><option>ME</option><option>PF</option><option>Outro</option></select></label>
        <label className="field"><span>Temperatura</span><select value={form.temp} onChange={e=>patch('temp',e.target.value)}><option value="hot">Quente</option><option value="warm">Morno</option><option value="cold">Frio</option></select></label>
      </div>
      <div className="section-heading"><h3>Serviços da oportunidade</h3><p>Selecione um ou mais serviços e ajuste os valores apenas se necessário.</p></div>
